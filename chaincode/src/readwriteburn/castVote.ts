...
export async function castVote(ctx: GalaChainContext, dto: CastVoteDto): Promise<void> {
  ...
  if (RWB_TYPES[entryType] === undefined) throw new ValidationFailedError(`VoteDto missing entryType`);

  const existingEntry: ChainObject & IEntry = await getObjectByKey(ctx, RWB_TYPES[entryType], entry);
