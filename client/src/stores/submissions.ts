...
export const useSubmissionsStore = defineStore("submissions", () => {
  ...
  async function verifyContent(submissionId: number): Promise<ContentVerificationResponse> {
    try {
      const response = await fetch(
        `${import.meta.env.VITE_PROJECT_API}/api/submissions/${submissionId}/verify`
      );
  ...
  async function castVote(submissionId: number, amount: number, metamaskClient: any) {
    try {
      ...
      const response = await fetch(
        `${import.meta.env.VITE_PROJECT_API}/api/submissions/${submissionId}/vote`,
        {
          method: "POST",
          headers: {
            "Content-Type": "application/json"
  ...
  function updateSubmissionModerationStatus(
    id: number,
    status: SubmissionResponse["moderation_status"],
    verified: boolean
  ) {
