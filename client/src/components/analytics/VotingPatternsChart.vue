<template>
  <div class="voting-patterns-chart">
    ...
    <div v-else>
      ...
      <div class="space-y-2">
        <div v-for="hourData in data" :key="hourData.hour" class="flex items-center gap-3">
          ...
          <div class="flex-1 bg-gray-200 rounded h-4 relative overflow-hidden">
            ...
            <div class="absolute inset-0 flex items-center px-2">
              <span class="text-xs text-gray-700 font-medium"> {{ hourData.votes }} votes </span>
            </div>
          </div>
          <div class="w-16 text-xs text-gray-500 text-right">{{ hourData.avgAmount.toFixed(1) }} avg</div>
        </div>
      </div>
      ...
      <div class="mt-4 pt-4 border-t border-gray-200">
        ...
        <div class="text-sm text-gray-600">
          ...
          <p>
            Highest avg amount: {{ formatHour(highestAvgHour.hour) }} ({{
              highestAvgHour.avgAmount.toFixed(1)
            }}
            GALA)
          </p>
        </div>
...
const peakHour = computed(() => {
  return (
    props.data?.reduce((max, current) => (current.votes > max.votes ? current : max), props.data[0]) || {
      hour: 0,
      votes: 0,
      avgAmount: 0
    }
...
const highestAvgHour = computed(() => {
  return (
    props.data?.reduce(
      (max, current) => (current.avgAmount > max.avgAmount ? current : max),
      props.data[0]
