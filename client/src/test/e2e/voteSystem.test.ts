...
describe("Vote System End-to-End Tests", () => {
  ...
  describe("Fire Voting", () => {
    it("should successfully vote on a fire using chain key lookup", async () => {
      ...
      expect(global.fetch).toHaveBeenCalledWith(`${apiBase}/api/fires/${fireSlug}/chain-key`);
  ...
  describe("Submission Voting", () => {
    it("should successfully vote on a top-level submission", async () => {
      ...
      // Verify chain key lookup
      expect(global.fetch).toHaveBeenCalledWith(`${apiBase}/api/submissions/${submissionId}/chain-key`);
  ...
  describe("Vote DTO Validation", () => {
    ...
    it("should reject invalid vote quantities", () => {
      ...
      expect(VoteService.validateVoteRequest(invalidRequest)).toBe("Vote quantity must be greater than 0");
    });
    ...
    it("should reject missing user authentication", () => {
      ...
      expect(VoteService.validateVoteRequest(invalidRequest)).toBe("User must be authenticated to vote");
    });
  });
  ...
  describe("Error Handling", () => {
    ...
    it("should handle signing errors", async () => {
      ...
      // Mock signing error
      mockMetamaskClient.signVote.mockRejectedValueOnce(new Error("User rejected signature"));
